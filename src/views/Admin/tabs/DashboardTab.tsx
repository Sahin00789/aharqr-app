...
export default function AdminDashboard() {
    ...
    async function loadDashboardMetrics() {
      try {
        ...
        if (tablesRes.success && tablesRes.tables) {
          ...
          const occ = tablesRes.tables.filter((t: any) => t.isOccupied || t.status === 'OCCUPIED').length;
          setOccupiedTables(occ || 3);
        }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      ...
      <div className="bg-linear-to-br from-slate-900 to-slate-950 border border-emerald-500/30 rounded-3xl p-6 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">{occupiedTables} out of {totalTables} tables currently occupied with live KOT orders.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="text-left md:text-right border-l md:border-l-0 md:border-r border-slate-800 pl-4 md:pl-0 md:pr-6">
            ...
            <span className="text-2xl font-mono font-black text-emerald-400">{occupiedTables} / {totalTables} Tables</span>
          </div>
          <Link
            to="/admin/tables"
