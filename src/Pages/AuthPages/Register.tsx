...
export default function Register() {
  ...
  // Step 1: Google Code in state (NOT sent to backend for creation yet!)
  ...
  const startGoogleVerification = useGoogleLogin({
    ...
    onSuccess: async (codeResponse) => {
      ...
      try {
        ...
        if (isAxiosError(err)) {
          setError(err.response?.data?.error || err.response?.data?.message || 'Failed to fetch Google profile details.');
        }
      } finally {
    ...
    <main className="min-h-[100dvh] bg-slate-950 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950 flex flex-col items-center justify-center p-3 sm:p-4 text-slate-200 font-sans selection:bg-blue-500/30 relative">
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-xl">
        ...
        <div className="bg-slate-900/80 backdrop-blur-2xl border border-slate-800/90 rounded-[32px] p-6 sm:p-8 shadow-2xl relative overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 text-center">
              ...
              {!googleCode ? (
                <button
                  ...
                  className="w-full flex items-center justify-center py-3.5 px-4 rounded-2xl text-xs font-bold bg-white text-slate-950 hover:bg-slate-100 shadow-xl transition-all border border-slate-200 active:scale-[0.98]"
                >
                  <GoogleIcon /> Authenticate Admin Identity with Google
                </button>
              ) : isVerifyingGoogle ? (
                <div className="bg-slate-950/80 p-6 rounded-2xl border border-slate-800 flex items-center justify-center gap-3">
                  <Loader2 className="w-5 h-5 text-blue-400 animate-spin" />
                  <span className="text-xs text-slate-300 font-semibold">Fetching Google Admin Profile...</span>
                </div>
              ) : googleProfile ? (
                /* GOOGLE ADMIN PROFILE DISPLAY CARD */
                <div className="bg-gradient-to-br from-blue-500/10 via-slate-950 to-indigo-500/10 border border-blue-500/30 rounded-3xl p-5 text-left space-y-4 shadow-lg">
                  ...
                  {!accountAlreadyExists ? (
                    <p className="text-xs text-slate-300 italic border-t border-slate-800/80 pt-3">
                      Google OAuth verified! Code saved in state. Proceed to Step 2 to enter restaurant details.
                    </p>
                  ) : (
              ...
              <div className="pt-4 flex justify-end">
                <button
                  type="button"
                  disabled={!googleCode || accountAlreadyExists || isVerifyingGoogle}
                  onClick={() => setStep(2)}
                  className={`py-3 px-6 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all ${
                    googleCode && !accountAlreadyExists && !isVerifyingGoogle
          ...
          {step === 2 && (
            <motion.form initial={{ opacity: 0 }} animate={{ opacity: 1 }} onSubmit={handleNextToPlanInfo} className="space-y-4">
              <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <Building2 className="w-5 h-5 text-blue-400" /> Step 2: Restaurant & Address
                </h2>
                <span className="text-xs text-slate-400">Pincode Auto-Lookup</span>
              </div>

              {/* PINCODE AUTO-LOOKUP FIELD */}
              <div className="bg-slate-950/90 p-3.5 rounded-2xl border border-blue-500/30 space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-white flex items-center gap-1.5">
                    <Compass className="w-4 h-4 text-blue-400" /> Enter 6-Digit Postal Code (Auto-Fill Address) *
                  </label>
                  {isPincodeLoading && (
                    <span className="text-[11px] text-blue-400 flex items-center gap-1">
                      <Loader2 className="w-3 h-3 animate-spin" /> Lookup...
                    </span>
                  )}
                </div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white font-mono placeholder-slate-600 focus:outline-none focus:border-blue-500"
                  />
                </div>

                {pincodeSuccessMsg && (
                  <p className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1 mt-1">
                    <CheckCircle2 className="w-3.5 h-3.5" /> {pincodeSuccessMsg}
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Building / Premises *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  ...
                  {postOfficeOptions.length > 0 ? (
                    <select
                      ...
                      className="w-full bg-slate-950 border border-blue-500/50 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-blue-500 font-semibold"
                    >
              ...
              <div className="pt-4 flex justify-between">
                ...
                <button
                  type="submit"
                  className="py-2.5 px-5 rounded-xl text-xs font-bold bg-blue-600 hover:bg-blue-500 text-white flex items-center gap-2 shadow-lg"
                >
          ...
          {step === 3 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
              ...
              <div className="bg-gradient-to-br from-amber-500/15 via-orange-500/10 to-amber-600/5 border border-amber-500/40 rounded-3xl p-5 space-y-4 shadow-[0_0_30px_-5px_rgba(245,158,11,0.25)]">
              ...
              <div className="pt-2 flex justify-between items-center">
                ...
                <button
                  ...
                  className="py-3 px-6 rounded-2xl text-xs font-bold bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white flex items-center gap-2 shadow-lg shadow-emerald-500/25 active:scale-[0.98]"
                >
